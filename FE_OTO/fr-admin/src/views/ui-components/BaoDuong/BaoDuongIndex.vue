<template>
  <div>
    <div class="mt-4">
      <h1 class="heading-tb mt-3">Thông tin bảo dưỡng</h1>
    </div>
    <!-- <div>
    ...
    <v-table>
      <thead>
        <tr>
          ...
          <th class="text-subtitle-1 font-weight-bold text-center font-weight-black">Tên sản phẩm</th>
          <th class="text-subtitle-1 font-weight-bold text-center font-weight-black">Ngày bảo dưỡng</th>
          <th class="text-subtitle-1 font-weight-bold text-center font-weight-black">Tên cửa hàng</th>
          <!-- <th class="text-subtitle-1 font-weight-bold text-center font-weight-black">Chức năng</th> -->
          <!-- <th>Ghi chú</th> -->
        </tr>
      </thead>
      <tbody>
        <tr v-for="item in desserts" :key="item.maBD">
          ...
          <td class="text-center">{{ item.ngayBD }}</td>
  ...
  <BaoDuongAdd :isOpen="isOpenAdd" @close="handleClose" @add="handleAdd" :data="dataAdd" />
  <BaoDuongEdit
    :isOpen="isOpenEdit"
    @update="handleUpdate"
...
import BaoDuongAdd from "./BaoDuongAdd.vue";
import BaoDuongEdit from "./BaoDuongEdit.vue";
const desserts = ref([]);
...
const getData = async () => {
  try {
    const res = await axios.get(
      `http://localhost:5222/api/BaoDuong/GetPageBaoDuong?page=${page.value}&pageSize=5`
    );
